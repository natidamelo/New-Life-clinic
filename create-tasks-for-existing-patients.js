...
async function createTasksForExistingPatients() {
  try {
    ...
    // Find Samuel and Natan
    const samuelPatient = existingPatients.find(p => 
      p.firstName.toLowerCase().includes('samuel') || 
      p.lastName.toLowerCase().includes('negatu')
    );
    
    const natanPatient = existingPatients.find(p => 
      p.firstName.toLowerCase().includes('natan') || 
      p.lastName.toLowerCase().includes('kinfe')
    );
    ...
    if (!natanPatient) {
      console.log('❌ Natan Kinfe not found in patients');
      return;
    }

    console.log(`✅ Found Samuel: ${samuelPatient.firstName} ${samuelPatient.lastName}`);
    console.log(`✅ Found Natan: ${natanPatient.firstName} ${natanPatient.lastName}`);
...
createTasksForExistingPatients();
