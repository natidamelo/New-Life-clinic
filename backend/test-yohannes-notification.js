...
mongoose.connect('mongodb://localhost:27017/clinic-cms')
  .then(async () => {
    ...
    try {
      // Clean up existing test notifications for Yohannes
      await Notification.deleteMany({ 
        'data.patientName': 'yohannes sami',
        'data.prescriptionId': '6888a3a95e0c5b28bf46ebcd'
      });
      console.log('✅ Cleaned up existing test notifications for Yohannes');
      
      // Yohannes's most recent prescription data
      const yohannesPrescription = {
        prescriptionId: '6888a3a95e0c5b28bf46ebcd',
        patientId: '6884fed411bf30c11e59ec6a',
      ...
      // Process medications for notification
      const notificationMedications = await Promise.all(yohannesPrescription.medications.map(async (med) => {
        let estimatedCost = 0;
        let sellingPrice = 0;
        let totalDoses = 1;
        ...
        return {
          ...
          prescriptionId: yohannesPrescription.prescriptionId
        };
      }));
      ...
      if (overallTotalAmount > 0) {
        const receptionNotification = new Notification({
          ...
          message: `Medication payment required for ${yohannesPrescription.patientName}. Total amount: ${overallTotalAmount}`,
          data: {
            prescriptionId: yohannesPrescription.prescriptionId,
            patientId: yohannesPrescription.patientId,
            patientName: yohannesPrescription.patientName,
        ...
        try {
          ...
          console.log(`✅ Notification created successfully for Yohannes with ${notificationMedications.length} medications.`);
          console.log(`✅ Notification ID: ${receptionNotification._id}`);
          console.log(`✅ Patient: ${yohannesPrescription.patientName}`);
