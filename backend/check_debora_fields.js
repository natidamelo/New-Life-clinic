...
async function run() {
  try {
    ...
    if (debora) {
      console.log('Debora Fields:', JSON.stringify({
        status: debora.status,
        hasFinalizedRecord: debora.hasFinalizedRecord,
        assignedDoctorId: debora.assignedDoctorId,
        vitals: debora.vitals,
        services: debora.services
      }, null, 2));
    }
