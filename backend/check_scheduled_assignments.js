...
async function run() {
  try {
    ...
    console.log(`Scheduled Patients: ${scheduled.length}`);
    
    scheduled.forEach(p => {
      console.log(`- ${p.firstName} ${p.lastName}, assignedTo: ${p.assignedDoctorId}`);
    });

    const natanId = '6823301cdefc7776bf7537b3';
    const scheduledToNatan = scheduled.filter(p => p.assignedDoctorId === natanId);
    console.log(`Scheduled patients assigned to Natan: ${scheduledToNatan.length}`);
