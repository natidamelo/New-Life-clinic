...
async function fixAndCreateProcedures() {
  try {
    // Connect to MongoDB
    ...
    // Find wound care service requests that have invoices (meaning they were paid)
    const woundCareRequests = await ServiceRequest.find({
    ...
    console.log(`🔍 Found ${woundCareRequests.length} wound care service requests with invoices`);

    if (woundCareRequests.length === 0) {
      console.log('❌ No wound care service requests with invoices found');
      return;
    }
    ...
    for (const serviceRequest of woundCareRequests) {
      try {
        ...
        console.log(`   Invoice: ${serviceRequest.invoice._id}`);
        ...
        // Update service request status to completed since it has an invoice (was paid)
        ...
        console.error(`   ❌ Error processing service request for ${serviceRequest.patient.firstName} ${serviceRequest.patient.lastName}:`, error.message);
      }
    }

    console.log(`\n🎉 Successfully processed ${woundCareRequests.length} service requests!`);
    console.log(`   ✅ Updated ${updatedServiceRequests.length} service request statuses`);
