...
async function fixNotificationPatientName() {
  try {
    ...
    const response = await fetch('http://192.168.78.157:5002/api/lab-orders/pending-for-reception');
    if (response.ok) {
      const labOrders = await response.json();
      console.log(`📊 API returned ${labOrders.length} pending lab orders`);
      if (labOrders.length > 0) {
        console.log(`   Sample: ${labOrders[0].testName} - ${labOrders[0].patient?.firstName} ${labOrders[0].patient?.lastName}`);
      }
    } else {
