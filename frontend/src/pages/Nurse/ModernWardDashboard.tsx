...
const quickActions = [
  {
    ...
    route: '/app/ward/ipd',
    color: 'bg-teal-600',
    lightBg: 'bg-teal-50 dark:bg-teal-500/20',
  ...
  },
  {
    ...
    color: 'bg-rose-500',
    lightBg: 'bg-rose-50 dark:bg-rose-500/20',
  ...
  },
  {
    ...
    color: 'bg-violet-500',
    lightBg: 'bg-violet-50 dark:bg-violet-500/20',
  ...
  },
  {
    ...
    color: 'bg-amber-500',
    lightBg: 'bg-amber-50 dark:bg-amber-500/20',
  ...
  },
  {
    ...
    color: 'bg-cyan-500',
    lightBg: 'bg-cyan-50 dark:bg-cyan-500/20',
  ...
  },
  {
    ...
    color: 'bg-indigo-500',
    lightBg: 'bg-indigo-50 dark:bg-indigo-500/20',
  ...
  },
  {
    ...
    color: 'bg-slate-500',
    lightBg: 'bg-slate-50 dark:bg-slate-500/20',
...
const priorityConfig: Record<string, { label: string; cls: string; dot: string }> = {
  urgent:  { label: 'Urgent',  cls: 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300', dot: 'bg-red-500' },
  high:    { label: 'High',    cls: 'bg-orange-100 text-orange-700 dark:bg-orange-500/20 dark:text-orange-300', dot: 'bg-orange-500' },
  medium:  { label: 'Medium',  cls: 'bg-blue-100 text-blue-700 dark:bg-blue-500/20 dark:text-blue-300', dot: 'bg-blue-500' },
  normal:  { label: 'Normal',  cls: 'bg-slate-100 text-slate-600 dark:bg-slate-500/20 dark:text-slate-300', dot: 'bg-slate-400' },
  low:     { label: 'Low',     cls: 'bg-slate-100 text-slate-500 dark:bg-slate-500/20 dark:text-slate-300', dot: 'bg-slate-300' },
};
      ...
      <div className="space-y-2 max-h-72 overflow-y-auto pr-1">
        {list.map(task => {
          ...
          return (
            <div
              key={task.id}
              className="flex items-start gap-3 p-3 rounded-xl border border-border hover:border-border/80 hover:bg-accent/10 transition-all group"
            >
              <div className={`w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0 mt-0.5
                ${task.type === 'medication' ? 'bg-primary/10 text-primary' :
                  task.type === 'vitals'     ? 'bg-rose-100 text-rose-500 dark:bg-rose-500/20 dark:text-rose-300' :
                  task.type === 'injection'  ? 'bg-violet-100 text-violet-500 dark:bg-violet-500/20 dark:text-violet-300' :
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  {task.dueTime && (
                    <>
                      ...
                      <span className="text-xs text-muted-foreground">{formatDate(task.dueTime)}</span>
                    </>
                  )}
                </div>
              </div>
              ...
              {task.isIpd && (
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-teal-100 text-teal-700 dark:bg-teal-500/20 dark:text-teal-300">IPD</span>
              )}
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-0.5">
              {greeting()}, <span className="font-medium text-foreground">{user?.firstName} {user?.lastName}</span>
          ...
          <div className="flex items-center gap-2">
            {stats.urgentTasks > 0 && (
              <div className="flex items-center gap-1.5 px-3 py-1.5 bg-red-50 border border-red-200 rounded-xl text-xs font-semibold text-red-700 dark:bg-red-500/20 dark:border-red-500/30 dark:text-red-300">
            ...
            <button
              ...
              disabled={refreshing}
              className="flex items-center gap-2 px-4 py-2 bg-card border border-border rounded-xl text-sm font-medium text-foreground hover:bg-accent/20 transition-colors shadow-sm"
            >
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map(card => (
            <div key={card.label} className={`bg-card rounded-2xl border ${card.border} p-4 shadow-sm`}>
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm p-4">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className={`text-sm font-bold ${completionRate >= 80 ? 'text-emerald-600' : completionRate >= 50 ? 'text-amber-600' : 'text-muted-foreground'}`}>
            ...
            {quickActions.map(action => {
              ...
              return (
                <button
                  ...
                  className="group bg-card rounded-2xl border border-border p-4 text-left hover:border-border/80 hover:shadow-md transition-all duration-150 active:scale-[0.98]"
                >
        ...
        <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
          ...
          <div className="flex border-b border-border">
            {([
              { key: 'pending',   label: 'Pending',   count: stats.pendingTasks,   color: 'text-amber-600 dark:text-amber-300' },
              { key: 'overdue',   label: 'Overdue',   count: stats.overdueCount,   color: 'text-red-600 dark:text-red-300' },
              { key: 'completed', label: 'Completed', count: stats.completedTasks, color: 'text-emerald-600 dark:text-emerald-300' },
            ] as const).map(tab => (
              <button
                key={tab.key}
              ...
              >
                ...
                {!loading && tab.count > 0 && (
                  <span className={`text-xs font-bold px-1.5 py-0.5 rounded-full
                    ${activeTab === tab.key
                      ? tab.key === 'overdue' ? 'bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-300'
                        : tab.key === 'completed' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300'
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: 'Vitals Needed',  value: stats.vitalsNeeded,   icon: Heart,         color: 'text-rose-600 dark:text-rose-300',   bg: 'bg-rose-50 dark:bg-rose-500/20' },
            { label: 'Injections',     value: stats.injectionTasks, icon: Syringe,       color: 'text-violet-600 dark:text-violet-300', bg: 'bg-violet-50 dark:bg-violet-500/20' },
            { label: 'Overdue',        value: stats.overdueCount,   icon: AlertTriangle, color: 'text-red-600 dark:text-red-300',    bg: 'bg-red-50 dark:bg-red-500/20' },
          ].map(item => (
