...
const BillingDashboard: React.FC = () => {
  ...
  const handleTabClick = (tabKey: string) => {
    navigate(getPathFromTab(tabKey));
  };
  ...
  const totalInvoices = stats ? (Object.values(stats.invoicesCount).reduce((a: any, b: any) => a + b, 0) as number) : 0;
  ...
  return (
    <div className="min-h-screen bg-slate-50/30 dark:bg-slate-950/20 pb-12">
      {/* ── Persistent Navigation Tabs Bar ── */}
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-gray-200/50 dark:border-slate-800/50 shadow-sm py-3 px-6 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            <span className="font-bold text-gray-900 dark:text-white text-base mr-3 hidden sm:block">Finance Center</span>
                  ...
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-bold transition-all duration-150 ${
                      isActive
                        ...
                        : 'text-gray-650 dark:text-slate-400 hover:bg-gray-100 dark:hover:bg-slate-850 hover:text-gray-900 dark:hover:text-slate-200'
                    }`}
                  >
                    {tab.icon}
                    <span>{tab.label}</span>
                        ...
                        return (
                          <div key={label}>
                            <div className="flex justify-between text-xs mb-1">
                              <span className="text-gray-650 font-medium">{label}</span>
