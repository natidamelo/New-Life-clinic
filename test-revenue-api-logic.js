...
// Helper: extract the clean base drug/test name
...
async function run() {
  ...
  try {
    ...
    // Scenario 1: Default date range (1 year before 2026-05-22)
    ...
    console.log("Total raw rows matching query:", rawRows.length);
    ...
    // Let's find Basic patient card membership
    Object.values(grouped).forEach(g => {
      if (g.baseName.toLowerCase().includes('basic')) {
        console.log(`\nFound basic card match: "${g.baseName}" (itemType: "${g.itemType}")`);
        console.log(`Total Revenue: ${g.totalRevenue}`);
        console.log(`Total Quantity: ${g.totalQuantity}`);
        console.log(`Invoice Count: ${g.invoiceCount}`);
        console.log("Monthly breakdown:", JSON.stringify(g.monthMap, null, 2));
      }
    });
